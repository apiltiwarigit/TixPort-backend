...
class EventsController {
  ...
  async getEventsByCategory(req, res) {
    try {
      ...
      const pageNum = Math.max(1, parseInt(page));
      const limitNum = Math.min(100, Math.max(1, parseInt(limit)));
      const result = await ticketEvolutionService.getEventsByCategory(parseInt(categoryId), pageNum, limitNum);
      res.json({
        success: true,
        data: result
      });
  ...
  async getEventsByLocation(req, res) {
    try {
      ...
      const pageNum = Math.max(1, parseInt(page));
      const limitNum = Math.min(100, Math.max(1, parseInt(limit)));
      const result = await ticketEvolutionService.getEventsByLocation(city, state, pageNum, limitNum);
      res.json({
        success: true,
        data: result
      });
  ...
  async searchEvents(req, res) {
    try {
      ...
      const pageNum = Math.max(1, parseInt(page));
      const limitNum = Math.min(100, Math.max(1, parseInt(limit)));
      const result = await ticketEvolutionService.searchEvents(q.trim(), pageNum, limitNum);
      res.json({
        success: true,
        data: result
      });
  ...
  async getEventsByPerformer(req, res) {
    try {
      ...
      const pageNum = Math.max(1, parseInt(page));
      const limitNum = Math.min(100, Math.max(1, parseInt(limit)));
      const result = await ticketEvolutionService.getEventsByPerformer(parseInt(performerId), pageNum, limitNum);
      res.json({
        success: true,
        data: result
      });
  ...
  async getEventsByVenue(req, res) {
    try {
      ...
      const pageNum = Math.max(1, parseInt(page));
      const limitNum = Math.min(100, Math.max(1, parseInt(limit)));
      const result = await ticketEvolutionService.getEventsByVenue(parseInt(venueId), pageNum, limitNum);
      res.json({
        success: true,
        data: result
      });
